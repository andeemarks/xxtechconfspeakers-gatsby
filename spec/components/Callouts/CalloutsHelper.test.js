...
test('#findMostRecentlyAddedConference does as the name suggests :-)', t => {
  t.deepEqual(helper.findMostRecentlyAddedConference([{
    node: {
      dateAdded: 2000
    }
  }, {
...
test('#findConfsForCurrentYear does as the name suggests :-)', t => {
  t.deepEqual(helper.findConfsForCurrentYear([{
    node: {
      year: 2017
    }
  }, {
...
test('#calculateAverageDiversity does as the name suggests :-)', t => {
  t.is(helper.calculateAverageDiversity([{
      node: {
  ...
  t.is(helper.calculateAverageDiversity([{
      node: {
        diversityPercentage: .25
      }
    },
    {
      node: {
        diversityPercentage: .75
      }
    }
  ]), .5);
...
test('#confFromCurrentYear can determine whether a conf year is in the current year', t => {
  t.false(helper.confFromCurrentYear({
    node: {
      year: 2000
    }
  }));

  t.true(helper.confFromCurrentYear({
    node: {
      year: 2018
    }
  }));
...
test('#diversitySorter returns a value determining the relative order of diversityPercentage of its arguments', t => {
  ...
  t.is(helper.diversitySorter({ diversityPercentage: 0.49 }, { diversityPercentage: 0.49 }), 0);
  t.is(helper.diversitySorter({ diversityPercentage: 0.49 }, { diversityPercentage: 0.50 }), 1);
  t.is(helper.diversitySorter({ diversityPercentage: 0.49 }, { diversityPercentage: 0.48 }), -1);
...
test('#dateAddedSorter returns a value determining the relative order of dateAdded of its arguments', t => {
  ...
  t.is(helper.dateAddedSorter({ node: { dateAdded: 2000 } }, { node: { dateAdded: 2000 } }), 0);
  t.is(helper.dateAddedSorter({ node: { dateAdded: 2000 } }, { node: { dateAdded: 2001 } }), 1);
  t.is(helper.dateAddedSorter({ node: { dateAdded: 2001 } }, { node: { dateAdded: 2000 } }), -1);
