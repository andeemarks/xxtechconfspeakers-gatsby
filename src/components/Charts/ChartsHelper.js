...
class ChartsHelper {
  ...
  countConfsByDiversityCohort(confs) {
    var confsByDiversityCohort = _.groupBy(confs, function(conf) {
    ...
    return [
      this.populateCohort(
        confsByDiversityCohort['5'],
        ' confs >= 50%',
        'white'
      ),
      this.populateCohort(
        confsByDiversityCohort['4'],
        ' confs >= 40%',
        'green'
      ),
      this.populateCohort(confsByDiversityCohort['3'], ' confs >= 30%', 'blue'),
      this.populateCohort(
        confsByDiversityCohort['2'],
        ' confs >= 20%',
        'orange'
      ),
      this.populateCohort(
        confsByDiversityCohort['1'],
        ' confs >= 10%',
        'fuchsia'
      ),
